...
class TmsValenciaportsBackend(models.Model):
    ...
    product_id = fields.Many2one(
        comodel_name="product.product", string="PCS Product", required=True,
    )
    ...
    def _search_or_create_container_type(self, name, iso_type):
        ...
        if not size_type:
            size_type = SizeType.new({"code": iso_type, "name": name,})
    ...
    def _search_or_create_equipment(self, name, size_type):
        ...
        if not equipment:
            equipment = Equipment.create({"name": name, "size_type_id": size_type.id,})
        return equipment
